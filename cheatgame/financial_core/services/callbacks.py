...
def _transport_rejection(*, method, content_type, body, headers):
...
def ingest_callback_delivery(
    *,
    ...
    transport_reason = _transport_rejection(
        method=method,
...
def _persist_normalized(
    *,
    ...
    existing_event = ProviderEvent.objects.filter(deduplication_identity=dedupe).first()
    contradiction = bool(existing_event and existing_event.canonical_envelope_hash != raw_hash)
    ...
    ).select_related("attempt__payment").first()
    quarantine_reason = ""
    ...
    if existing_event:
        ...
        work_id = None
        if contradiction and existing_event.transaction_id:
            work, _ = enqueue_verification_work(
                transaction_obj=existing_event.transaction,
                provider_event=existing_event,
                work_type=VerificationWorkType.ESCALATE_UNKNOWN_OUTCOME,
                deterministic_identity=f"callback-contradiction:{existing_event.public_id}",
                correlation_id=receipt.correlation_id,
                causation_id=existing_event.public_id,
            )
            work_id = work.pk
        return CallbackIngestionResult(receipt, existing_event, work_id)
